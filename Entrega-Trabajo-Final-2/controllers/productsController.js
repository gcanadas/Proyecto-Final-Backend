...
class productsController {
    ...
    async readArchive (archiveName) {
        try {
            const data = await promises.readFile(archiveName, encoding);
    ...
    async writeArchive (archiveName, content) {
        try {
            await promises.writeFile(archiveName, JSON.stringify(content, null, 2));
        } catch (err) {
    ...
    async deleteAll() {
        try {
            await promises.unlink(this.archiveName);
